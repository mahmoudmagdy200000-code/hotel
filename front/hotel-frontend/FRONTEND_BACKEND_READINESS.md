...
| CORS for Vite (Dev-only) | **MISSING** | **Critical:** Frontend will be blocked by CORS without this. |
...
| Attachment view/stream | **MISSING** | **Required:** No way to view uploaded PDF in the UI yet. |
| Stable DateOnly contract | **PARTIAL** | Some DTOs use `DateTime` where `DateOnly` is preferred. |
| Seed data exists | **PASS** | Good base set of rooms and reservations. |
